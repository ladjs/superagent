...
Request.prototype.end = function(fn){
  ...
  switch (method) {
    ...
    default:
      ...
      if ('string' != typeof data) {
        var serialize = self._serialize || exports.serialize[req.getHeader('Content-Type')];
  ...
  req.on('response', function(res){
    ...
    // override with request specific parser if one is defined
    if (typeof self._parse === 'function') {
      parse = self._parse;
    }
...
/**
 * Handler for custom body parsing for response
 *
 * @param {function} parse
 * @return {Request} for chaining
 */
Request.prototype.parse = function(parse){
    this._parse = function(res, fn){
        res.text = '';
...
/**
 * Handler for custom serialization to server.
 * @param serialize
 * @return {*}
 */
Request.prototype.serialize = function(serialize){
