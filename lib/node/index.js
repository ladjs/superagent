...
exports.parse = {
  'application/x-www-form-urlencoded': function(req, fn){
    var buf = '';
    req.on('data', function(chunk){ buf += chunk.toString('ascii'); });
    req.on('end', function(){
      try {
  ...
  'application/json': function(req, fn){
    var decoder = new StringDecoder('utf8')
      , buf = '';
    req.on('data', function(chunk){ buf += decoder.write(chunk); });
    req.on('end', function(){
      try {
...
Request.prototype.end = function(fn){
  ...
  req.on('response', function(res){
    ...
    if (buffer) {
      var decoder = new StringDecoder('utf8');
      res.text = '';
      dataStream.on('data', function(chunk){ res.text += chunk.toString('utf8'); });
    }

    // parser
    var parse = exports.parse[utils.type(res.headers['content-type'] || '')];
    if (parse) {
      parse(dataStream, function(err, obj){
        // TODO: handle error
        res.body = obj;
      });
    }
    ...
    // end event
    self.res = res;
    dataStream.on('end', function(){ self.emit('end'); });
  });
