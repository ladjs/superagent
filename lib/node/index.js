...
Request.prototype.end = function(fn){
  ...
  if (this.fields.length) this.writeFields();
...
/**
 * Calculate the content length for all attachments and fields
 *
 * @param {Function} fn
 * @return {Request} for chaining
 * @api public
...
Request.prototype.contentLength = function(fn) {
  var data = this._data
    , self = this
    , req = this.request()
    , contentLength = 0
  ...
  // if(data) {
  //   addToContentLength(data);
  // }

  buildPartHeader = function(field, val) {
    return field + ': ' + val + '\r\n';
  };
...
Request.prototype.writeFields = function(){
  var fields = this.fields
    , req = this.request()
    , self = this;

  fields.forEach(function(field) {
    field.part.name(field.name).write(field.value)
  });
};
