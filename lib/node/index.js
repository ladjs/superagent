...
Request.prototype.appendQueryString = function(req){
  var queryArr = qs.stringify(this.qs, { indices: false }).split('&').concat(this.qsRaw);
  if (!queryArr[0]) {
    queryArr.shift();
  }
  if (this._sort) {
    ...
    if(~index){
      queryArr = queryArr.concat(req.path.substring(index + 1).split('&'));
      req.path = req.path.substring(0, index);
    }
    queryArr.sort(isFunction(this._sort)? this._sort : null);
  }
